...
export function HackathonCard({ hackathon }: { hackathon: Hackathon }) {
  ...
  const fallbackParticipants =
    10 +
    ([...hackathon.id].reduce((acc, ch) => acc + ch.charCodeAt(0), 0) % 50);
    ...
    >
      <div className="relative h-[200px] w-full bg-[#f5f7f9] overflow-hidden">
        ...
        <div className="absolute bottom-4 left-5">
          <span
            className={`px-4 py-1.5 text-[13px] font-extrabold rounded-full tracking-wide shadow-sm ${statusColor}`}
          >
      ...
      <div className="p-7 flex flex-col flex-1">
        ...
        <div className="flex flex-col gap-1.5 mb-6 text-sm text-[#595c5e] font-medium">
          <div className="flex items-center gap-2">
            ...
            <span>
              {new Date(
                hackathon.start_at || hackathon.submission_deadline_at,
              ).toLocaleDateString("ko-KR")}{" "}
              ~ {new Date(hackathon.end_at).toLocaleDateString("ko-KR")}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <svg
            ...
            <span>
              현재 참가자 수:{" "}
              {hackathon.participants_count ?? fallbackParticipants}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mt-auto pt-2">
          {hackathon.tags.map((tag) => (
            <span
              key={tag}
              className="text-[13px] font-bold text-[#9a9d9f] bg-[#f5f7f9] px-3 py-1.5 rounded-xl"
            >
              #{tag}
            </span>
          ))}
