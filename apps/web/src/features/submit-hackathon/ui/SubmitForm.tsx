import {
  createSubmission,
  fetchMySubmissionsForHackathon,
...
import {
  ...
  type SubmitHackathonFormValues,
} from "../model/schema";
...
export function SubmitForm({ hackathonId, onSuccess }: SubmitFormProps) {
  ...
  const { data: existingSubmissions = [], isLoading: submissionLoading } =
    useQuery({
  ...
  if (!user) {
    return (
      <div className="text-center py-16">
        <p className="text-xl font-bold text-[#2c2f31] mb-3">
          로그인이 필요합니다
        </p>
  ...
  if (mutation.isSuccess || existingSubmissions.length > 0) {
    const sub = existingSubmissions[0] || mutation.data;
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-10">
        ...
        <h3 className="text-3xl font-extrabold text-[#2c2f31] mb-4">
          제출이 완료되었습니다!
        </h3>
        <p className="text-[#595c5e] mb-10 text-lg">
          결과는 리더보드에서 확인하실 수 있습니다.
        </p>

        <div className="bg-[#f5f7f9] p-8 rounded-[2rem] text-left border border-slate-100 shadow-sm">
          <div className="grid gap-6">
            <div>
              <p className="text-xs font-bold text-[#9a9d9f] uppercase tracking-widest mb-2">
                프로젝트 링크
              </p>
              <a
                href={payload.project_url}
                target="_blank"
                rel="noreferrer"
                className="text-[#0064ff] font-bold text-lg hover:underline break-all"
              >
                {payload.project_url}
              </a>
            </div>
            {payload.demo_url && (
              <div>
                ...
                <a
                  href={payload.demo_url}
                  target="_blank"
                  rel="noreferrer"
                  className="text-[#0064ff] font-bold hover:underline"
                >
                  {payload.demo_url}
                </a>
              </div>
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#f5f7f9] p-5 rounded-2xl border border-[#eef1f3]">
        ...
        <ul className="space-y-2 text-sm text-[#595c5e] font-medium">
          <li className="flex gap-2">
            <span className="text-[#9a9d9f]">•</span> 데모 영상 링크는 가가급적
            3분 이내로 제작을 권장합니다.
          </li>
          <li className="flex gap-2">
            <span className="text-[#9a9d9f]">•</span> Github 저장소를 제출하실
            경우 반드시 Public 상태인지 확인해주세요.
          </li>
          <li className="flex gap-2">
            <span className="text-[#9a9d9f]">•</span> 여러 개의 파일은 ZIP으로
            압축하거나 구글 드라이브 링크로 제출하세요.
          </li>
        </ul>
      </div>
      ...
      <div>
        <label className="block text-sm font-bold text-[#2c2f31] mb-2">
          팀 회고 및 추가 메모{" "}
          <span className="text-[#9a9d9f] font-normal">(선택)</span>
        </label>
        <textarea
          {...register("memo")}
          rows={3}
