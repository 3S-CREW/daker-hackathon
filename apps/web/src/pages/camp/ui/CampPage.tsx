...
function DirectMessageModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center bg-[#2c2f31]/60 backdrop-blur-sm p-4"
      onClick={onClose}
    >
      <div
        className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl animate-in fade-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        <h3 className="text-xl font-bold text-[#2c2f31] mb-2">쪽지 보내기</h3>
        <p className="text-[#595c5e] text-sm mb-6">
          수신자:{" "}
          <span className="font-bold text-[#0064ff]">{receiverName}</span>
        </p>
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-3 bg-[#f5f7f9] text-[#595c5e] font-bold rounded-xl hover:bg-slate-200 cursor-pointer"
          >
            취소
          </button>
          <button
            ...
            className="flex-1 py-3 bg-[#0064ff] text-white font-bold rounded-xl hover:bg-[#0051d2] disabled:opacity-50 cursor-pointer shadow-lg shadow-blue-100"
          >
            {isSending ? "전송 중..." : "보내기"}
          </button>
        </div>
    ...
    <>
      <div className="fixed bottom-10 left-10 z-40">
        <button
          ...
          className="bg-white border-2 border-[#eef1f3] shadow-lg text-[#2c2f31] w-14 h-14 rounded-full hover:scale-105 transition-transform flex items-center justify-center relative cursor-pointer"
        >
          <svg
            className="w-6 h-6 text-[#595c5e]"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
      ...
      {open && (
        <>
          ...
          <div className="fixed bottom-28 left-10 z-50 w-80 sm:w-96 bg-white rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] border border-slate-100 overflow-hidden flex flex-col max-h-[60vh] animate-in slide-in-from-bottom-5 duration-300">
            <div className="bg-[#f5f7f9] p-4 flex justify-between items-center border-b border-slate-200">
              <h3 className="font-extrabold text-[#2c2f31] flex items-center gap-2">
                받은 쪽지함{" "}
                <span className="px-2 py-0.5 bg-[#0064ff] text-white text-xs rounded-full">
                  {inboxMessages.length}
                </span>
              </h3>
              <button
                onClick={() => setOpen(false)}
                className="text-[#9a9d9f] hover:text-[#2c2f31] cursor-pointer"
              >
                <svg
                  className="w-5 h-5"
                  fill="none"
            ...
            <div className="overflow-y-auto p-4 space-y-3 custom-scrollbar flex-1">
              {inboxMessages.length === 0 ? (
                <p className="text-center text-sm text-[#9a9d9f] py-10 font-medium">
                  받은 쪽지가 없습니다.
                </p>
              ) : (
                inboxMessages.map((m) => (
                  <div
                    key={m.id}
                    className="bg-[#f5f7f9] p-4 rounded-2xl relative"
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <img
                        src={
                          m.sender?.avatar_url || "https://github.com/ghost.png"
                        }
                      ...
                      <span className="text-[10px] text-[#9a9d9f] ml-auto">
                        {new Date(m.created_at).toLocaleDateString()}
                      </span>
                    </div>
                    <p className="text-sm text-[#595c5e] break-words whitespace-pre-wrap">
                      {m.content}
                    </p>
                  </div>
                ))
...
export function CampPage() {
  ...
  // DM / ExternalLink Modal State
  const [dmModalInfo, setDmModalInfo] = useState<{
    open: boolean;
    receiverId: string;
    receiverName: string;
  }>({
  ...
  const deleteTeamMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["teams"] });
      toast.success("팀 모집 게시물이 삭제되었습니다.");
    },
    onError: () => {
  ...
  const filteredTeams = teams.filter((t) => {
    ...
    if (selectedHackathonId && t.hackathon_id !== selectedHackathonId)
      return false;
    return true;
  });
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-72 bg-white rounded-3xl animate-pulse border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.03)]"
            />
          ))}
        </div>
        ...
        <div className="text-center py-20 bg-slate-50/50 rounded-3xl border border-slate-100">
          ...
          <p className="text-[#595c5e] font-medium mb-6">
            원하는 포지션이 없다면 직접 팀을 만들어보세요!
          </p>
          <button
            onClick={() => {
            ...
            className="bg-[#0064ff] text-white font-bold px-6 py-3 rounded-full hover:bg-[#0051d2] transition-colors shadow-md cursor-pointer"
          >
            팀 개설하기
          </button>
        </div>
          ...
          {filteredTeams.map((team) => {
            ...
            return (
              ...
              >
                    ...
                    {isMyTeam && (
                      <div className="flex gap-1 mr-1">
                        <button
                          ...
                          className="p-1.5 text-[#595c5e] hover:bg-[#f5f7f9] rounded-lg transition-colors cursor-pointer"
                        >
                          <svg
                            width="16"
                            height="16"
                ...
                <p className="text-[#595c5e] text-sm leading-relaxed mb-6 flex-1 line-clamp-3 font-medium">
                  {team.intro}
                </p>
                ...
                <div className="flex flex-col gap-4 mb-6">
                  <div className="flex items-center gap-2 text-[#595c5e] font-bold text-sm bg-[#f5f7f9] px-3 py-2 rounded-xl w-fit">
                    ...
                    {team.member_count}{" "}
                    <span className="text-[#9a9d9f] font-medium text-xs">
                      / {team.max_members}
                    </span>
                  </div>
                  <div className="flex gap-1.5 flex-wrap">
                    {team.looking_for.map((role) => (
                      <span
                        key={role}
                        className="text-[11px] font-extrabold text-[#2c2f31] bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-200/60 shadow-sm"
                      >
                        {role}
                      </span>
                    ))}
                ...
                {isMyTeam ? (
                  <button
                    onClick={() =>
                      toggleRecruitMutation.mutate({
                        teamId: team.id,
                        current: team.recruiting,
                      })
                    }
                    className={`w-full py-3.5 font-extrabold rounded-2xl transition-all border-2 flex items-center justify-center gap-2 cursor-pointer ${
                      team.recruiting
                        ? "border-[#eef1f3] text-[#595c5e] bg-white hover:bg-slate-50"
                        : "border-transparent text-white bg-slate-800 hover:bg-slate-700 shadow-lg shadow-slate-200"
                    }`}
                  >
                  ...
                  <div className="flex gap-2">
                    {team.contact_url && (
                      <button
                        ...
                        className="flex-1 py-3.5 text-center text-[#0064ff] bg-blue-50/50 hover:bg-blue-100 font-extrabold rounded-2xl transition-colors border border-blue-100 cursor-pointer"
                      >
                        지원하기
                      </button>
                    )}
                    <button
                      onClick={() =>
                        setDmModalInfo({
                          ...
                          receiverName: team.name,
                        })
                      }
                      className="px-4 py-3.5 text-white bg-[#0064ff] hover:bg-[#0051d2] font-extrabold rounded-2xl transition-transform shadow-[0_8px_20px_rgba(0,100,255,0.25)] hover:-translate-y-0.5 cursor-pointer"
                    >
                      <svg
                        width="20"
                        height="20"
