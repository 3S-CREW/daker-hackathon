    ...
    <header className="sticky top-0 z-50 w-full bg-white/80 backdrop-blur">
      <div className="container mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-8 font-semibold text-[15px] text-[#595c5e]">
          <Link
            to="/hackathons"
            className="hover:text-[#0064ff] transition-colors"
          >
            Hackathons
          </Link>
          <Link to="/camp" className="hover:text-[#0064ff] transition-colors">
            Camp
          </Link>
          <Link
            to="/rankings"
          ...
          {!isLoading &&
            (user ? (
              <div className="flex items-center gap-4">
                <Link
                  to="/dashboard"
                  className="text-sm font-semibold text-[#2c2f31] hover:text-[#0064ff] transition-colors"
                >
                  {user.user_metadata?.user_name || user.email}
                </Link>
                <button
                  onClick={handleLogout}
                  className="px-4 py-2 text-sm font-bold text-[#595c5e] bg-slate-100 hover:bg-slate-200 rounded-full transition-colors active:scale-95"
                >
                  로그아웃
                </button>
              </div>
